...
;(function($) {
    ...
    $.fn.instagramLite = function(options) {
	    ...
	    var formatCaption = function(caption) {
            ...
            newCaption = newCaption.slice(0,50) + "...";
						        	...
						        	success: function(data) {
						        		...
						        		if(data.meta.code === 200 && data.data.length) {
							        		...
							        		for(var i = 0; i < data.data.length; i++) {
							        			...
							        			var thisMedia = data.data[i],item='',itemDiv='',imgDiv='',divUrl='',cap='<p class="para">Lorem ipsum dolor sit conse ctetur amet adipiscing elit.</p>',dt='';
							        			...
							        			if(thisMedia.type === 'image' || !s.videos) {
                                                    ...
                                                    imgDiv = '<div class="thumb"><img class="img-responsive" src="'+thisMedia.images.standard_resolution.url+'" alt="featured project"></div>';

								        			// if url setting is true
								        			if(s.urls) {

								        				item = '<a href="'+thisMedia.link+'" target="_blank">'+item+'</a>';
                                                        divUrl = '<div class="icons twin"><a href="'+thisMedia.images.standard_resolution.url+'" data-lightbox="featured-projects"><i class="fa fa-search"></i></a> <a href="'+thisMedia.link+'" target="_blank"><i class="fa fa-link"></i></a></div>';

								        			}

								        			if(s.captions || s.date || s.likes) {
									        			item += '<div class="il-photo__meta">';
                                                        itemDiv = '<div class="overlay"><div class="details">';
								        			}
								        			...
								        			if(s.captions && thisMedia.caption) {
                                                        ...
                                                        cap = '<p class="para">'+formatCaption(thisMedia.caption.text)+'</p>';
								        			...
								        			if(s.date) {
                                                        ...
                                                        dt = '<h4 class="title"><center>'+date+'</center></h4>';
								        			...
								        			if(s.captions || s.date || s.likes) {
									        			item += '</div>';
                                                        itemDiv = itemDiv+dt+cap+divUrl+'</div></div>';
								        			}
                                                ...
                                                if (s.div && itemDiv) {
                                                    itemDiv = '<div class="item box-hover-effect '+s.effect+'">'+imgDiv+itemDiv+'</div>';
                                                    //console.log(itemDiv);
                                                } else if(s.list && item) {
